...
namespace log
{
   inline FILE* Init()
   {
      ...
      sprintf(tmp, "./logs/ls_%d%02i%02i.log",  timeinfo->tm_year+1900, timeinfo->tm_mon+1, timeinfo->tm_mday);
   ...
   {
	   HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
	   SetConsoleTextAttribute(hConsole, (WORD)FOREGROUND_RED | FOREGROUND_BLUE);
	   va_list ap;
	   va_start(ap, format);
